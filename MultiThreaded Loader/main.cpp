	...
	case WM_COMMAND:
	{
		...
		case ID_FILE_LOADIMAGE:
		{
			if (ChooseImageFilesToLoad(_hwnd))
			{
				...
				/*
					for number of elements in img vector
						hBMP = (HBITMAP) LoadImage( NULL, "Your/ImagePath/a.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
						create sprite -> push hBMP
						push sprite to sprite vec
